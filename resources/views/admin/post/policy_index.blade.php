                    ...
                    @foreach (config('admin.post.policy.status') as $key => $value)
                      <td class="align-middle text-center">
                        <div class="custom-control custom-checkbox">
                          ...
                          @endphp
                          <input type="checkbox" id="update-status-{{$key}}-{{$k}}" class="update-status custom-control-input" name="{{ $key }}" data-id="{{ $row->id }}" data-url="{{route('admin.policy.update_status')}}" {{ in_array($key, $status) ? 'checked' : '' }} />
