  ...
  <section class="content">
    <form action="{{ route($route, ['type' => $type, 'id' => !empty($row->id) ? $row->id : '']) }}" class="validation-form" method="POST" enctype="multipart/form-data">
      ...
      <div class="card-footer text-sm sticky-top">
        ...
        @if ($row)
          <a href="{{route('admin.page.remake', ['type' => $type, 'id' => $row->id, 'hash' => $row->hash])}}" class="btn btn-sm bg-gradient-secondary">
                    ...
                    <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                      <div class="tab-pane fade show active" id="tabs-sluglang-vi" role="tabpanel" aria-labelledby="tabs-lang">
                        <div class="form-gourp mb-0">
                          ...
                          <input type="text" class="slug-seo form-control slug-input no-validate text-sm" name="slug" id="slug" placeholder="Đường dẫn mẫu" value="{{!empty($row->slug) ? $row->slug : ''}}"/>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="tab-pane fade show active" id="tabs-lang-vi" role="tabpanel" aria-labelledby="tabs-lang">
                      ...
                      @if(config($sloganConfig) === true)
                        <div class="form-group">
                          ...
                          <input type="text" class="for-seo form-control text-sm" name="slogan" id="slogan" placeholder="Slogan" value="{{!empty($row->slogan) ? $row->slogan : ''}}"/>
                        </div>
                      @endif
                      @if(config($titleConfig) === true)
                        <div class="form-group">
                          ...
                          <input type="text" class="for-seo form-control text-sm" name="title" id="title" placeholder="Tiêu đề" value="{{!empty($row->title) ? $row->title : ''}}"/>
                          @error('title')
                      ...
                      @if (config($descriptionConfig) === true)
                        <div class="form-group">
                          ...
                          <textarea name="desc" class="form-control text-sm {{ config($descriptionTinyConfig) === true ? 'tiny' : ''}}" id="desc" cols="30" rows="10" placeholder="Mô tả">{!! !empty($row->desc) ? $row->desc : '' !!}</textarea>
                        </div>
                      @endif
                      ...
                      @if (config($contentConfig) === true)
                        <div class="form-group">
                          ...
                          <textarea name="content" class="form-control text-sm {{ config($contentTinyConfig) === true ? 'tiny' : ''}}" id="content" cols="30" rows="10" placeholder="Nội dung">{!! !empty($row->content) ? $row->content : '' !!}</textarea>
                        </div>
                      @endif
                    </div>
                  </div>
              ...
              <div class="card-body">
                <div class="photoUpload-zone">
                  <div class="photoUpload-detail" id="photoUpload-preview1">
                    @if (!empty($row->photo1))
                      <img class="rounded img-preview img-fluid" src="{{url("public/upload/page/$row->photo1")}}" alt="{{$row->photo1}}"/>
                      <a class="delete-photo" href="{{ route($deletePhoto, ['type' => $type,'id' => $row->id, 'action' => 'photo1']) }}" style="cursor: pointer" title="Xóa hình ảnh">
                        <i class="far fa-trash-alt text-white"></i>
                      </a>
                    @else
                      <img class="rounded img-preview img-fluid" src="{{ url($noimage) }}" alt="Hình ảnh 1"/>
                    @endif
                  </div>
              ...
              <div class="card-body">
                <div class="photoUpload-zone">
                  <div class="photoUpload-detail" id="photoUpload-preview2">
                    @if (!empty($row->photo2))
                      <img class="rounded img-preview img-fluid" src="{{url("public/upload/page/$row->photo2")}}" alt="{{$row->photo2}}"/>
                      <a class="delete-photo" href="{{ route($deletePhoto, ['id' => $row->id, 'action' => 'photo2']) }}" style="cursor: pointer" title="Xóa hình ảnh">
                        <i class="far fa-trash-alt text-white"></i>
                      </a>
                    @else
                      <img class="rounded img-preview img-fluid" src="{{ url($noimage) }}" alt="Hình ảnh 1"/>
              ...
              <div class="card-body">
                <div class="photoUpload-zone">
                  <div class="photoUpload-detail" id="photoUpload-preview3">
                    @if (!empty($row->photo3))
                      <img class="rounded img-preview img-fluid" src="{{url("public/upload/page/$row->photo3")}}" alt="{{$row->photo3}}"/>
                      <a class="delete-photo" href="{{ route($deletePhoto, ['id' => $row->id, 'action' => 'photo3']) }}" style="cursor: pointer" title="Xóa hình ảnh">
                        <i class="far fa-trash-alt text-white"></i>
                      </a>
                    @else
                      <img class="rounded img-preview img-fluid" src="{{ url($noimage) }}" alt="Hình ảnh 1"/>
              ...
              <div class="card-body">
                <div class="photoUpload-zone">
                  <div class="photoUpload-detail" id="photoUpload-preview4">
                    @if (!empty($row->photo4))
                      <img class="rounded img-preview img-fluid" src="{{url("public/upload/page/$row->photo4")}}" alt="{{$row->photo4}}"/>
                      <a class="delete-photo" href="{{ route($deletePhoto, ['id' => $row->id, 'action' => 'photo4']) }}" style="cursor: pointer" title="Xóa hình ảnh">
                        <i class="far fa-trash-alt text-white"></i>
                      </a>
                    @else
                      <img class="rounded img-preview img-fluid" src="{{ url($noimage) }}" alt="Hình ảnh 1"/>
