...
@section('content')
  ...
  <section class="content">
    {!! Form::open(['name' => 'form-newsletter-list', 'route' => ['admin.newsletter.sendmail'], 'class' => ['form-newsletter-list', 'form-newsletter-request']]) !!}
      <div class="card-footer text-sm sticky-top">
        <button type="submit" name="send-mail" class="btn btn-sm bg-gradient-success text-white" id="send-email" title="Gửi email">
          <i class="fas fa-paper-plane mr-2"></i>Gửi email
        </button>
        <a class="btn btn-sm bg-gradient-primary text-white" href="{{route('admin.newsletter.create')}}" title="Thêm mới">
          <i class="fas fa-plus mr-2"></i>Thêm mới
        </a>
        <a data-url="{{route('admin.newsletter.destroy')}}" class="btn btn-sm bg-gradient-danger text-white delete-all-request" id="delete-all">
              ...
              @if ($rows->total() > 0)
                @foreach ($rows as $k => $row)
                  <tr>
                    ...
                    <td class="align-middle">
                      <input type="number" class="update-num-newsletter form-control form-control-mini m-auto" min="0" value="{{ $row->num }}" data-token="{{ csrf_token() }}" data-id="{{ $row->id }}" data-url="{{ route('admin.newsletter.update_number') }}"/>
                    </td>

                    @if (config('admin.message.newsletter.fullname') === true)
                      <td class="align-middle">
                        <a href="{{ route('admin.newsletter.show', [$row->id]) }}" class="text-dark text-break" title="{{$row->fullname}}">
                          {{$row->fullname}}
                        </a>
                      </td>
                    @endif
                    ...
                    @if (config('admin.message.newsletter.email') === true)
                      <td class="align-middle">
                        <a href="{{ route('admin.newsletter.show', [$row->id]) }}" class="text-dark text-break" title="{{$row->email}}">
                          {{$row->email}}
                        </a>
                      </td>
                    @endif
                    ...
                    @if (config('admin.message.newsletter.file_attach') === true)
                      <td class="align-middle">
                        @if (!empty($row->file_attach))
                          <a class="btn btn-sm bg-gradient-primary text-white d-inline-block p-1 rounded" href="{{asset('upload/newsletter_file_attach/'.$row->file_attach)}}" target="_blank" title="Download tập tin"><i class="fas fa-download mr-2"></i>Download tập tin</a>
                        @else
                    ...
                    <td class="align-middle text-center text-md text-nowrap">
                      <a class="text-primary mr-2" href="{{ route('admin.newsletter.show', [$row->id]) }}" title="Chỉnh sửa">
                        <i class="fas fa-edit"></i>
                      </a>
                      <a class="text-danger delete-row" data-url="{{route('admin.newsletter.delete', ['id' => $row->id])}}" title="Xóa" style="cursor: pointer">
      ...
      <div class="card card-primary card-outline text-sm mb-0 mt-3">
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <input type="text" name="subject" id="subject" class="form-control text-sm" placeholder="Tiêu đề"/>
          </div>
          ...
          @if (config('admin.message.newsletter.content') === true)
            <div class="form-group">
              ...
              <textarea class="form-control {{config('admin.message.newsletter.content_tiny') === true ? 'tiny' : ''}}" name="content" id="content" rows="5" placeholder="Nội dung thông tin"></textarea>
            </div>
          @endif
        </div>
      </div>
