                    ...
                    @foreach (config('admin.news.category.category1.status') as $key => $value)
                      <td class="align-middle text-center">
                        <div class="custom-control custom-checkbox">
                          ...
                          @endphp
                          <input type="checkbox" id="update-status-{{$key}}" class="update-status custom-control-input" name="{{ $key }}" data-id="{{ $row->id }}" data-url="{{route('admin.category_news1.update_status')}}" {{ in_array($key, $status) ? 'checked' : '' }} />
