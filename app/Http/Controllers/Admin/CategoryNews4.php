...
class CategoryNews4 extends Controller
{
  ...
  public function __construct()
  {
    ...
    $this->numberPerPage = config('admin.news.category.category4.number_per_page');
    $this->uploadCategoryNews = "public/upload/category_news4";
    $this->with1 = config("admin.news.category.category4.width1");
    $this->with2 = config("admin.news.category.category4.width2");
    $this->with3 = config("admin.news.category.category4.width3");
    $this->with4 = config("admin.news.category.category4.width4");
    $this->height1 = config("admin.news.category.category4.height1");
    $this->height2 = config("admin.news.category.category4.height2");
    $this->height3 = config("admin.news.category.category4.height3");
    $this->height4 = config("admin.news.category.category4.height4");
  }
  ...
  public function index(Request $request)
  {
    ...
    if ($request->input('keyword')) {
      $rows = CategoryNews::where("title", "LIKE", "%{$request->input('keyword')}%")->where('type', $this->typeCategory4)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage);
    } else {
      $rows = CategoryNews::where('type', $this->typeCategory4)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage);
    }
  ...
  public function update(Request $request)
  {
    ...
    $categoryProduct = CategoryNews::where('type', $this->typeCategory4)->find($request->id);
    $dataSeo = [
      'title_seo' => !empty($request->input('title_seo')) ? htmlspecialchars($request->input('title_seo')) : null,
      'keywords' => !empty($request->input('keywords')) ? htmlspecialchars($request->input('keywords')) : null,
      'description_seo' => !empty($request->input('description_seo')) ? htmlspecialchars($request->input('description_seo')) : null,
  ...
  public function delete($id, $hash)
  {
    ...
    CategoryNews::where('type', $this->typeCategory4)->where('hash', $hash)->delete($id);
    $seo->delete();
