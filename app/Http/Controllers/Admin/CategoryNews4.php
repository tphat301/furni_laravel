...
class CategoryNews4 extends Controller
{
  ...
  {
    session(['module_active' => 'category_news_level4_index']);
    if ($request->input('keyword')) {
      $rows = CategoryNews::where("title", "LIKE", "%{$request->input('keyword')}%")->where('type', config('admin.news.category.category4.type'))->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate(config('admin.news.category.category4.number_per_page'));
    } else {
      $rows = CategoryNews::where('type', config('admin.news.category.category4.type'))->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate(config('admin.news.category.category4.number_per_page'));
    }
    return view('admin.category_news_level4.index', compact('rows'));
  }

  /* Category news create */
  public function create()
  {
    $rowCategory1 = CategoryNews::where('type', config('admin.news.category.category4.type'))->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    session(['module_active' => 'category_news_level4_create']);
    return view('admin.category_news_level4.create', compact('rowCategory1'));
  }
  ...
  public function save(Request $request)
  {
    ...
    if ($this->helper->hasFile("photo1")) {
      $photo1 = $this->helper->uploadFile("photo1", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news4/");
    }
    if ($this->helper->hasFile("photo2")) {
      $photo2 = $this->helper->uploadFile("photo2", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news4/");
    }
    if ($this->helper->hasFile("photo3")) {
      $photo3 = $this->helper->uploadFile("photo3", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news4/");
    }
    if ($this->helper->hasFile("photo4")) {
      $photo4 = $this->helper->uploadFile("photo4", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news4/");
    }
    if (!$validator->fails()) {
      $data = [
        ...
        'type' => config('admin.news.category.category4.type'),
  ...
  {
    $row = CategoryNews::where('type', config('admin.news.category.category4.type'))->find($request->id);
    $rowSeo = Seo::where('type', config('admin.news.category.category4.type'))->where('hash_seo', $row->hash)->first();
    $rowCategory1 = CategoryNews::where('type', config('admin.news.category.category4.type'))->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    return view('admin.category_news_level4.show', compact('row', 'rowSeo', 'rowCategory1'));
  }
  ...
  public function update(Request $request)
  {
    ...
    $categoryProduct = CategoryNews::where('type', config('admin.news.category.category4.type'))->find($request->id);
    $dataSeo = [
      'title_seo' => !empty($request->input('title_seo')) ? htmlspecialchars($request->input('title_seo')) : null,
      'keywords' => !empty($request->input('keywords')) ? htmlspecialchars($request->input('keywords')) : null,
      'description_seo' => !empty($request->input('description_seo')) ? htmlspecialchars($request->input('description_seo')) : null,
    ...
    return $this->helper->transfer("Cập nhật dữ liệu", "success", route('admin.category_news4.show', ['id' => $categoryProduct->id]));
  }

  /* Category news duplicate */
  public function copy($id)
  {
    $row = CategoryNews::where('type', config('admin.news.category.category4.type'))->find($id);
  ...
  public function delete($id, $hash)
  {
    ...
    $categoryNews = CategoryNews::where('type', config('admin.news.category.category4.type'))->where('hash', $hash)->find($id);
    $seo = SEO::where('type', config('admin.news.category.category4.type'))->where('hash_seo', $hash);
    ...
    CategoryNews::where('type', config('admin.news.category.category4.type'))->where('hash', $hash)->delete($id);
    $seo->delete();
  ...
  {
    $upload = "public/upload/category_news4/";
    $categoryNews = CategoryNews::where('type', config('admin.news.category.category4.type'))->find($request->checkitem);
    ...
    Seo::where('type', config('admin.news.category.category4.type'))->whereIn('hash_seo', $request->hashes)->delete();
  ...
  {
    CategoryNews::where('id', $request->id)->where('type', config('admin.news.category.category4.type'))->update(['num' => $request->value]);
  }

  /* Category news update status ajax */
  public function updateStatus(Request $request)
  {
    $status = CategoryNews::select('status')->where('type', config('admin.news.category.category4.type'))->find($request->id)->status;
    ...
    CategoryNews::where('id', $request->id)->where('type', config('admin.news.category.category4.type'))->update(['status' => $statusStr]);
  }
  ...
  public function deletePhoto(Request $request)
  {
    ...
    $photo = CategoryNews::where('type', config('admin.news.category.category4.type'))->find($request->id)->$action;
    $photo = isset($photo) && !empty($photo) ? $photo : "";
    if (file_exists($upload . $photo) && !empty($photo)) unlink($upload . $photo);
    CategoryNews::where('id', $request->id)->where('type', config('admin.news.category.category4.type'))->update([$action => null]);
