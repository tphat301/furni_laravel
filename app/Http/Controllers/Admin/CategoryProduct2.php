...
class CategoryProduct2 extends Controller
{
  ...
  {
    $rowCategory1 = CategoryProduct::where('type', config('admin.product.category.category2.type'))->where('level', 1)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    session(['module_active' => 'category_product_level2_create']);
    return view('admin.category_product_level2.create', compact('rowCategory1'));
  }
  ...
  public function save(Request $request)
  {
    ...
    if ($this->helper->hasFile("photo1")) {
      $photo1 = $this->helper->uploadFile("photo1", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_product2/");
    }
    if ($this->helper->hasFile("photo2")) {
      $photo2 = $this->helper->uploadFile("photo2", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_product2/");
    }
    if ($this->helper->hasFile("photo3")) {
      $photo3 = $this->helper->uploadFile("photo3", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_product2/");
    }
    if ($this->helper->hasFile("photo4")) {
      $photo4 = $this->helper->uploadFile("photo4", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_product2/");
    }
    if (!$validator->fails()) {
      $data = [
        ...
        'type' => config('admin.product.category.category2.type'),
  ...
  {
    $row = CategoryProduct::where('type', config('admin.product.category.category2.type'))->find($request->id);
    $rowSeo = Seo::where('type', config('admin.product.category.category2.type'))->where('hash_seo', $row->hash)->first();
    $rowCategory1 = CategoryProduct::where('type', config('admin.product.category.category2.type'))->where('level', 1)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    return view('admin.category_product_level2.show', compact('row', 'rowSeo', 'rowCategory1'));
  }
  ...
  public function update(Request $request)
  {
    ...
    $categoryProduct = CategoryProduct::where('type', config('admin.product.category.category2.type'))->find($request->id);
    $dataSeo = [
      'title_seo' => !empty($request->input('title_seo')) ? htmlspecialchars($request->input('title_seo')) : null,
      'keywords' => !empty($request->input('keywords')) ? htmlspecialchars($request->input('keywords')) : null,
      'description_seo' => !empty($request->input('description_seo')) ? htmlspecialchars($request->input('description_seo')) : null,
  ...
  {
    $row = CategoryProduct::where('type', config('admin.product.category.category2.type'))->find($id);
  ...
  public function delete($id, $hash)
  {
    ...
    $categoryProduct = CategoryProduct::where('type', config('admin.product.category.category2.type'))->where('hash', $hash)->find($id);
    $seo = SEO::where('type', config('admin.product.category.category2.type'))->where('hash_seo', $hash);
    ...
    CategoryProduct::where('type', config('admin.product.category.category2.type'))->where('hash', $hash)->delete($id);
    $seo->delete();
  ...
  {
    $upload = "public/upload/category_product2/";
    $categoryProduct = CategoryProduct::where('type', config('admin.product.category.category2.type'))->find($request->checkitem);
    ...
    Seo::where('type', config('admin.product.category.category2.type'))->whereIn('hash_seo', $request->hashes)->delete();
  ...
  {
    CategoryProduct::where('id', $request->id)->where('type', config('admin.product.category.category2.type'))->update(['num' => $request->value]);
  }

  /* Category product update status ajax */
  public function updateStatus(Request $request)
  {
    $status = CategoryProduct::select('status')->where('type', config('admin.product.category.category2.type'))->find($request->id)->status;
    ...
    CategoryProduct::where('id', $request->id)->where('type', config('admin.product.category.category2.type'))->update(['status' => $statusStr]);
  }
  ...
  public function deletePhoto(Request $request)
  {
    ...
    $photo = CategoryProduct::where('type', config('admin.product.category.category2.type'))->find($request->id)->$action;
    $photo = isset($photo) && !empty($photo) ? $photo : "";
    if (file_exists($upload . $photo) && !empty($photo)) unlink($upload . $photo);
    CategoryProduct::where('id', $request->id)->where('type', config('admin.product.category.category2.type'))->update([$action => null]);
