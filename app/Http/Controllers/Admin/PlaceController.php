...
class PlaceController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    if ($request->input('keyword')) {
      $rows = $namespace::where('name_' . $type, "LIKE", "%{$request->input('keyword')}%")->where('type_' . $type, $type)->orderBy('num', 'ASC')->orderBy('id_' . $type, 'ASC')->paginate($numberPerPage)->appends($appendQueryString);
    } else {
