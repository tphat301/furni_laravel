...
class CategoryNews3 extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    if ($request->input('keyword')) {
      $rows = CategoryNews::where("title", "LIKE", "%{$request->input('keyword')}%")->where('type', config('admin.news.category.category3.type'))->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate(config('admin.news.category.category3.number_per_page'));
    } else {
      $rows = CategoryNews::where('type', config('admin.news.category.category3.type'))->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate(config('admin.news.category.category3.number_per_page'));
    }
  ...
  {
    $rowCategory1 = CategoryNews::where('type', config('admin.news.category.category3.type'))->where('level', 2)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
  ...
  public function save(Request $request)
  {
    ...
    if ($this->helper->hasFile("photo1")) {
      $photo1 = $this->helper->uploadFile("photo1", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news3/");
    }
    if ($this->helper->hasFile("photo2")) {
      $photo2 = $this->helper->uploadFile("photo2", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news3/");
    }
    if ($this->helper->hasFile("photo3")) {
      $photo3 = $this->helper->uploadFile("photo3", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news3/");
    }
    if ($this->helper->hasFile("photo4")) {
      $photo4 = $this->helper->uploadFile("photo4", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "public/upload/category_news3/");
    }
    if (!$validator->fails()) {
      $data = [
        ...
        'type' => config('admin.news.category.category3.type'),
  ...
  {
    $row = CategoryNews::where('type', config('admin.news.category.category3.type'))->find($request->id);
    $rowSeo = Seo::where('type', config('admin.news.category.category3.type'))->where('hash_seo', $row->hash)->first();
    $rowCategory1 = CategoryNews::where('type', config('admin.news.category.category3.type'))->where('level', 2)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
  ...
  public function update(Request $request)
  {
    ...
    $categoryNews = CategoryNews::where('type', config('admin.news.category.category3.type'))->find($request->id);
    $dataSeo = [
      'title_seo' => !empty($request->input('title_seo')) ? htmlspecialchars($request->input('title_seo')) : null,
      'keywords' => !empty($request->input('keywords')) ? htmlspecialchars($request->input('keywords')) : null,
      'description_seo' => !empty($request->input('description_seo')) ? htmlspecialchars($request->input('description_seo')) : null,
  ...
  {
    $row = CategoryNews::where('type', config('admin.news.category.category3.type'))->find($id);
  ...
  public function delete($id, $hash)
  {
    ...
    $categoryNews = CategoryNews::where('type', config('admin.news.category.category3.type'))->where('hash', $hash)->find($id);
    $seo = SEO::where('type', config('admin.news.category.category3.type'))->where('hash_seo', $hash);
    ...
    CategoryNews::where('type', config('admin.news.category.category3.type'))->where('hash', $hash)->delete($id);
    $seo->delete();
  ...
  {
    $upload = "public/upload/category_news3/";
    $categoryNews = CategoryNews::where('type', config('admin.news.category.category3.type'))->find($request->checkitem);
    ...
    Seo::where('type', config('admin.news.category.category3.type'))->whereIn('hash_seo', $request->hashes)->delete();
  ...
  {
    CategoryNews::where('id', $request->id)->where('type', config('admin.news.category.category3.type'))->update(['num' => $request->value]);
  }

  /* Category news update status ajax */
  public function updateStatus(Request $request)
  {
    $status = CategoryNews::select('status')->where('type', config('admin.news.category.category3.type'))->find($request->id)->status;
    ...
    CategoryNews::where('id', $request->id)->where('type', config('admin.news.category.category3.type'))->update(['status' => $statusStr]);
  }
  ...
  public function deletePhoto(Request $request)
  {
    ...
    $photo = CategoryNews::where('type', config('admin.news.category.category3.type'))->find($request->id)->$action;
    $photo = isset($photo) && !empty($photo) ? $photo : "";
    if (file_exists($upload . $photo) && !empty($photo)) unlink($upload . $photo);
    CategoryNews::where('id', $request->id)->where('type', config('admin.news.category.category3.type'))->update([$action => null]);
