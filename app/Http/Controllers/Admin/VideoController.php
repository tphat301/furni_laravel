...
class VideoController extends Controller
{
  ...
  public function videoMultipleIndex(Request $request)
  {
    ...
    if ($request->input('keyword')) {
      $rows = Photo::where("title", "LIKE", "%{$request->input('keyword')}%")->where('type', $type)->where('action', $this->videoMultipleAction)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($numberPerPage);
    } else {
